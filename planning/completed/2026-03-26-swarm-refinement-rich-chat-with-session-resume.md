...
**Completed 2026-03-26. Tests: 184 backend + 37 frontend.**

### What was built

**Backend (Phase 1):**
- `orchestrator.py` — `synthesis_session_id` stored after synthesis, `chat()` method with `resume_session`, `_chat_lock` mutex, streaming deltas + tool events via `event_bus.emit_sync`
- `rest.py` — `POST /api/swarm/{swarm_id}/chat` (404/409/400/200, auth via existing middleware)
- `schemas.py` — `ChatRequest` with `Field(min_length=1, max_length=10000)`

**Frontend State (Phase 2):**
- `useChatState.ts` — Separate `chatReducer` with per-swarm chat state, streaming messages, tool tracking (9 tests)
- `types/swarm.ts` — `ChatMessage`, `ChatState`, `ChatStore` types

**Frontend Components (Phase 3):**
- `ChatPanel.tsx` — Full rich chat with user/assistant bubbles, streaming markdown, auto-scroll
- `ChatInput.tsx` — Textarea with Enter-to-send, disabled during streaming
- `StreamingMarkdown.tsx` — Progressive markdown rendering with safe flush points
- `ResizableLayout.tsx` — Two-column drag-to-resize layout
- `useAutoScroll.ts` — Smart auto-scroll with user-scroll-away detection
- `App.tsx` — Full-screen report+chat view replacing modal, chat event routing

### Live test findings

1. **Chat works end-to-end** — Synthesis agent responds to user questions with substantive analysis
2. **Synthesis agent hallucinates delegation** — Says "let me spin up Dr. Nakamura, Dr. Voss, Dr. Okafor" but has no ability to spawn agents. Pure hallucination. Fix planned: refinement mode prompt injection (Phase 3.5A)
3. **30-second delay before response** — Session resume + agent thinking time. Fix planned: thinking indicator (Phase 3.5B)
4. **Tool events flowing** — 12 tool calls logged during chat (CLI built-in tools like bash/view), but tool names not captured in forwarded events. Fix planned: better logging (backlog item)

### Remaining phases

- **Phase 3.5A:** Refinement prompt injection — tell synthesis agent it's in refinement mode, cannot delegate
- **Phase 3.5B:** Thinking indicator above chat input
- **Phase 4:** Mermaid diagram rendering
- **Phase 5:** Tool execution cards in chat
